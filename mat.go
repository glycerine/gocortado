...
// Addable is the
// type constraint for Matrix. Since
// we have an Add() method, the
// elements of the Matrix must support addition.
...
func (m *Matrix[T]) Transpose() {
	if m.IsSliceOfColVec {
		panic("m.IsSliceOfColVec is true, so Matrix[T].Transpose() is not allowed")
	}
...
// m is column major.
func (m *Matrix[T]) RowInto(i int, fillme []T) (res []T) {
	if i < 0 {
		panic("i must not be negative")
	}
	if i >= m.Nrow {
...
func (m *Matrix[T]) RowChunk(beg, endx int) (r *Matrix[T]) {
	...
	if m.IsColMajor || m.IsSliceOfColVec {
		...
		r = &Matrix[T]{Dat: m.Dat[beg*m.Ncol : endx*m.Ncol], Ncol: m.Ncol, Nrow: endx - beg, Colnames: m.Colnames}
		if len(m.Rownames) == m.Nrow {
			r.Rownames = m.Rownames[beg:endx]
		}
		return
...
func NewMatrixColMajor[T Addable](nrow, ncol int) (m *Matrix[T]) {
	m = NewMatrix[T](nrow, ncol)
	m.IsColMajor = true
	return
}
...
func NewMatrixColVec[T Addable](nrow, ncol int) (m *Matrix[T]) {
	m = &Matrix[T]{
		Nrow:            nrow,
		Ncol:            ncol,
		Dat:             make([]T, nrow*ncol),
...
func (m *Matrix[T]) Clone() (clone *Matrix[T]) {
	clone = &Matrix[T]{
		...
		ColVec: make([]*ColVec[T], len(m.ColVec)),
	...
	if m.IsSliceOfColVec {
		...
		nrow := m.Nrow
		clone.Dat = make([]T, nrow*clone.Ncol)
...
func (m *Matrix[T]) AddRow(rowlabel string) (i int) {
	...
	if m.IsSliceOfColVec {
		...
		m.Dat = append(m.Dat, make([]T, m.Ncol)...)
		i = m.Nrow
		m.Nrow++
	}
	if rowlabel == "" {
...
func (m *Matrix[T]) ReformatToSliceOfColVec() {
	if m.IsSliceOfColVec {
		// already there
		return
	}
...
func (m *Matrix[T]) Cbind(m2 *Matrix[T]) {
	...
	if m.IsSliceOfColVec {
		...
		if m2.IsSliceOfColVec {
			...
			for j := 0; j < m2.Ncol; j++ {
				m.ColVec = append(m.ColVec, &ColVec[T]{Dat: m2.Dat[j*m2.Nrow : (j+1)*m2.Nrow]})
			}
		}
	} else {
